...
## Prerequisites
In this tutorial we assume that you already have installed `npm` and `node` locally and that you already created an npm project.
...
### Npm project
If you don't already have an npm project initialised and it doesn't contain a `package.json` file, you can create one with the command
```bash
npm init
```
...
And then create a second filr called `page2.json` with this inside:
```json
...
Now we can call the library to generate the results.
...
Below there is an explenation of each property.
...
The link that this function returns will be used to open a page when the user clicks on a result. This means that it should point to the page created by the website generator.
```javascript
function generateLink(fileName, inputFolder){
    // ... [Creating link]
    return link
...
I suggest each link to use an absolute path instead of a relative one, this is to avoid issues related to the position of the search page.

#### outputFolder
The `outputFolder` property is the path to a directory that will be used to store the results that are generated.

The directory is refreshed each time we re-generate the results. Therefore it shouldn't be used for anything else.

#### Execute the library
Now that we know all the parameters we can use them to execute the library and generate the results.
...
This will scan through all the files and generate the results, indexing all the word inside each file present in the `inputFolder`.


## Showing the results
Now the results are ready to be used, and we'll need the second part of the library for it.

The library will show a searchbar where the user can search through your website. So we need a place where to show the searchbar.
...
### Add the searchbar
When Hulipaa is activated it will need a place where to add the searchbar and later on the results.

So we'll need to add in the `body` section of our page
```html
...
You can also wrap the searchbar in another div to control how much space it will occupy. By default it will always fill whatever space is available in the parent element.

Also the library will not create or modify any html that is outside the `hulipaa_searchbar` element.
...
#### resultsPath
The first property is `resultsPath` and is the path to the folder where we generated our results.
...
For this same reason I suggest to specify an absolute path, because a relative path might change depending on the location of the search page.
...
Hulipaa will call this function each time it needs to visualise a result. It will pass in the content of the data file and as a result it needs the text of the page, that will be shown as a preview of the result.

The function should return an object with the `text` property, containing the text of the page in plain text and without markup language.

The `parsePage` property is intenctionally compatible with the `parseData` property. So you can reuse the exact same function and avoid any dupliacted code.

#### Initialise the library
When we imported the library, a global function has been created called Hulipaa. We can call that function when our page is fully loaded and it will create the searchbar for us.
...
Rememebr to make sure the `hulipaa_searchbar` element has been loaded before calling the library. Usually you can achieve that by placing the script at the end of the body section, or by using the `DOMContentLoaded` event, like we did above.
...
### Open website
Now you can just open the browser and load your website, if you used `live-server` the url will be `http://localhost:8080`.
...
You should be able to see the title and a preview of each page. And clicking on the title of the page it should redirect to the page.

// TODO add screenshot of search page
...
* Generate the results alongside your website
* Add the library to your search page
* Specify where the results were saved
